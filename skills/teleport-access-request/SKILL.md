...
Each row carries a `ResourceID` and, when the cluster computed it, the
`Granted` and `Requestable` principal sets. A resource with only granted
principals is already usable without a request; one with requestable principals
is a candidate for an access request.

The split is not always present. An older cluster, or a listing that could not
compute it, omits `Granted`/`Requestable`. When they are missing, do not guess a
scope: request the whole resource unconstrained (see Step 3).
...
lists the `principal_kind` (`logins` for SSH nodes, `role_arns` for AWS console
apps), the `granted` set, and the `requestable` set. If the preview returns no
split, request the whole resource unconstrained rather than inventing a scope.
...
If a request carrying constraints fails to create because the cluster does not
support them, retry with the constraints removed. That requests broader,
unconstrained access to the whole resource, so tell the user that is what you
are doing.
