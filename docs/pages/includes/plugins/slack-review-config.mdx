...
Teleport user and Slack user: <Var name="slack_uid" />

The trait should hold value of Slack user ID: <Var name="U123456789" />
...
Configure the Slack plugin to notify you when a user requests the `access` role
...
above, requests for roles aside from `access` will notify the
