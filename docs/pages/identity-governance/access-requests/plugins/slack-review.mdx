---
title: Run the Slack Access Request Plugin with Access Request Reviews from Slack
description: How to set up Teleport's Slack plugin to review Access Requests within Slack.
sidebar_label: Slack with Access Request Reviews
tags:
 - access-requests
 - how-to
 - identity-governance
...
The Access Request reviews feature for the Teleport Slack plugin is currently
only for self-hosted plugins. Support for Teleport-hosted plugins will be added in the future.
</Admonition>

## How it works

Teleport's Slack integration notifies individuals and channels of
Access Requests. By enabling the plugin with Access Request reviews, users can then
...
In this step, you will configure your Slack app to support the Access Request reviews feature.
...
<Admonition type="info">
All users performing Access Request reviews within Slack must have an associated local Teleport user.
When using an external Identity Provider, users should be imported using
the User Sync feature from the Teleport Okta integration or the Teleport Entra ID integration.
...
We will assign the reviewer user with a Teleport trait named <Var name="slack_uid" />
...
Update the newly created user for <Var name="alice@example.com" />:
