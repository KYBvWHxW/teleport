...
func buildConstraintsFromSuffix(suffix string) (*types.ResourceConstraints, error) {
	// Values never contain "=", ",", or "|" (ARNs and system: names use ":" and
	// "/"), so splitting the suffix on "|" and each pair on the first "=" is
	// unambiguous.
...
func splitConstraintValues(key, raw string) ([]string, error) {
	...
	for _, p := range parts {
		...
		if p == "" {
			return nil, trace.BadParameter("constraint %q contains an empty value", key)
		}
		vals = append(vals, p)
	}
	if len(vals) == 0 {
		return nil, trace.BadParameter("constraint %q has no values", key)
	}
	return vals, nil
}
