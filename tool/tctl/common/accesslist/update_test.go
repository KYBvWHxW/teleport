...
func TestRejectUnknownRoles(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			newAl := newPresetAccessList("some-al-name")
...
func newPresetAccessList(name string) *accesslist.AccessList {
	reviewerRole := accesslist.RoleName(accesslist.RoleReviewerPrefix, name)
	requesterRole := accesslist.RoleName(accesslist.RoleRequesterPrefix, name)
