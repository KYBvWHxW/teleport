...
func rejectUnknownGrants(al *accesslist.AccessList) error {
	knownRoles := map[string]struct{}{
		accesslist.RoleName(accesslist.RoleReviewerPrefix, al.GetName()):  {},
		accesslist.RoleName(accesslist.RoleRequesterPrefix, al.GetName()): {},
		accesslist.RoleName(standardRolePrefixName, al.GetName()):         {},
		accesslist.RoleName(awsicRolePrefixName, al.GetName()):            {},
	}
	...
	filterKnownRoles := func(roles []string) []string {
		var filteredRoles []string
		for _, name := range roles {
			if _, ok := knownRoles[name]; ok {
				filteredRoles = append(filteredRoles, name)
			}
		}
	...
	validGrants := accesslist.Grants{
		Roles: filterKnownRoles(al.Spec.Grants.Roles),
	}
	validOwnerGrants := accesslist.Grants{
		Roles: filterKnownRoles(al.Spec.OwnerGrants.Roles),
	}

	unsupported := hasUnknownRole(al.PresetRoleNames()) ||
