...
type shouldRetryFunc func(context.Context, *libclient.TeleportClient, error) (error, bool)
...
// while holding an exclusive lock, while other conccurently executing tsh
...
func (s sshRetryer) acquireLock(ctx context.Context) (bool, func() error, error) {
	...
	// We don't necessarily _need_ a timeout. We will unblock when the exclusive lock holder
	// releases, exits, or the file is deleted. We'll try with a timeout anyway not because
	// we're concerned about blocking forever, but because FSTryReadLockTimeout is the only
	// API that exposes a context that we can cancel on.
...
func (s sshRetryer) retryWithLimitedRelogin(ctx context.Context, tc *libclient.TeleportClient, operation func() error) error {
	...
	if opErr, shouldRetry = s.shouldRetry(ctx, tc, opErr); !shouldRetry {
		return opErr
	}
