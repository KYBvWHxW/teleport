...
func TestAccessListService_ListAllAccessListMembers(t *testing.T) {
	...
	allMembers, err := stream.Collect(clientutils.Resources(t.Context(), service.ListAllAccessListMembers))
	require.NoError(t, err)
	...
	gotMembers, err := stream.Collect(clientutils.Resources(t.Context(), service.ListAllAccessListMembers))
	require.NoError(t, err)
	...
	gotMembers, err = stream.Collect(clientutils.Resources(t.Context(),
...
func TestAccessListService_ListAllAccessListReviews(t *testing.T) {
	...
	allReviews, err := stream.Collect(clientutils.Resources(ctx, service.ListAllAccessListReviews))
	require.NoError(t, err)
	...
	gotReviews, err := stream.Collect(clientutils.Resources(t.Context(), service.ListAllAccessListReviews))
	require.NoError(t, err)
	expectReviews(t, expectedReviews, gotReviews)

	// ListAllAccessListReviewsV2 should _not_ return the scoped list members if
	// they are filtered out.
	gotReviews, err = stream.Collect(clientutils.Resources(t.Context(),
