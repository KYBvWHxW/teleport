...
// is /scoped/scoped_role/role/<encoded_scope>/<name>.
func (k scopedRoleKey) Key() (backend.Key, error) {
	encodedScope, err := scopes.EncodeForKey(k.scope)
	if err != nil {
		return backend.Key{}, trace.Wrap(err)
	}
	return backend.NewKey(scopedNamespacePrefix, scopedRolePrefix, scopedRoleRoleComponent, encodedScope, k.name), nil
}

func scopedRoleWatchPrefix() backend.Key {
	return backend.ExactKey(scopedNamespacePrefix, scopedRolePrefix, scopedRoleRoleComponent)
}
...
// /scoped/scoped_role/assignment/<encoded_scope>/<name>/<sub_kind>.
func (k scopedRoleAssignmentKey) Key() (backend.Key, error) {
	...
	return backend.NewKey(scopedNamespacePrefix, scopedRolePrefix, scopedRoleAssignmentComponent, encodedScope, k.name, k.subKind), nil
}

func scopedRoleAssignmentWatchPrefix() backend.Key {
	return backend.ExactKey(scopedNamespacePrefix, scopedRolePrefix, scopedRoleAssignmentComponent)
}
