...
func TestGCPFetcher_CloudSQL(t *testing.T) {
	...
	tests := []struct {
		...
		{
			// The Cloud SQL API lists every region for a project at once, so
			// the fetcher filters by region client-side.
		...
		{
			// A wildcard project matcher expands to one fetcher per visible
			// project, and each fetcher discovers its own project's instances.
			name: "wildcard project expands to enumerated projects",
			instances: []*sqladmin.DatabaseInstance{
				makeInstance("db-1"),
...
func TestGCPFetcher_Accessors(t *testing.T) {
	...
	fetchers, err := MakeGCPFetchers(t.Context(), clients, []types.GCPMatcher{matcher}, discoveryConfigName)
...
func TestGCPFetcherConfig_LocationMatcher(t *testing.T) {
	t.Parallel()
	tests := []struct {
		name      string
		locations []string
		...
		{name: "empty matches all", locations: nil, region: "us-central1", want: true},
		{name: "wildcard matches all", locations: []string{types.Wildcard}, region: "us-central1", want: true},
		{name: "exact match", locations: []string{"us-central1"}, region: "us-central1", want: true},
		{name: "no match", locations: []string{"europe-west1"}, region: "us-central1", want: false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			cfg := gcpFetcherConfig{Locations: tt.locations}
			require.Equal(t, tt.want, cfg.locationMatcher()(tt.region))
		})
	}
}
...
func TestMakeGCPFetchers(t *testing.T) {
	...
	t.Run("one fetcher per project", func(t *testing.T) {
		...
		fetchers, err := MakeGCPFetchers(t.Context(), clients, []types.GCPMatcher{matcher}, "")
		require.NoError(t, err)
	...
	t.Run("non-database matcher type is rejected", func(t *testing.T) {
		...
		_, err := MakeGCPFetchers(t.Context(), clients, []types.GCPMatcher{matcher}, "")
		require.True(t, trace.IsBadParameter(err), "expected bad parameter, got %v", err)
	})
}
