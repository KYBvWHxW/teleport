...
func DiscoverCloudSQLDatabases(ctx context.Context, logger *slog.Logger, instances []*sqladmin.DatabaseInstance) []types.Database {
	...
	for _, instance := range instances {
		...
		if err != nil {
			...
			logger.WarnContext(ctx, "Failed to build database object", "name", instance.Name, "instance", instance, "error", err)
			continue
		}
		if skipReason != "" {
			logger.DebugContext(ctx, "Skipping Cloud SQL instance", "name", instance.Name, "reason", skipReason)
			continue
		}
