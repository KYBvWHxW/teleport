...
// ScopedCollection represents a complete batch of access from one source (e.g. entraID or okta)
...
// Where the relation between access list outside the ScopedCollection is not possible because collection source are
...
func (b *ScopedCollection) ListAccessListMembersV2(ctx context.Context, req *accesslistv1.ListAccessListMembersRequest) ([]*accesslist.AccessListMember, string, error) {
	...
	if req.GetPageSize() > 0 {
		endIdx = startIdx + int(req.GetPageSize())
