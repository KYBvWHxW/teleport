...
// scopedWatchKinds is the set of resource kinds that have been converted to support scoped watch
// (namespaced (scope, name) identity + scope-carrying events). A scope filter that targets scoped
// resources is only meaningful for these kinds.
var scopedWatchKinds = map[string]struct{}{
...
func (a *ScopedServerWithRoles) authorizeWatchKind(ctx context.Context, kind types.WatchKind) (*scopesv1.Filter, error) {
	...
	case types.KindAccessRequest, types.KindWebSession, types.KindHeadlessAuthentication:
		...
		// in the scoped model so that we can fully elmininate dependence on unscoped server with roles.
...
// unscopedWatchKindExceptions are *unscoped* kinds that permit scoped identities to read them via
// unpinned-read exceptions. This category is conceptually distinct from optionally scoped kinds
// like nodes, or scoped kinds that permit upward-reaching queries such as scoped roles.
var unscopedWatchKindExceptions = map[string]services.UnpinnedReadAuthorization{
	types.KindCertAuthority:    services.UnpinnedReadCertAuthority,
	types.KindSPIFFEFederation: services.UnpinnedReadSPIFFEFederation,
}

// authorizeUnscopedWatchKindException authorizes a scoped identity to watch the unscoped region of
// the specific named cluster-global kinds in unscopedWatchKindExceptions, which are readable
...
func (a *ScopedServerWithRoles) authorizeUnscopedWatchKindException(ctx context.Context, kind types.WatchKind) error {
	authorization, ok := unscopedWatchKindExceptions[kind.Kind]
	if !ok {
...
// are applied to select unscoped and parent-scped types that must be watchable because they have configuration
// or policy effects.
func (a *ScopedServerWithRoles) authorizeWatchKindCore(ctx context.Context, kind types.WatchKind) (*scopesv1.Filter, error) {
	if err := scopes.ValidateFilter(kind.ScopeFilter.ToProto()); err != nil {
		return nil, trace.Wrap(err)
	}
	...
	case scopesv1.Mode_MODE_UNSPECIFIED:
		...
		// for these types do not accept scope filter patameters makes us wary about doing that.
		if _, ok := unscopedWatchKindExceptions[kind.Kind]; ok {
	...
	// we need to explicitly reject attempts to sue a scope-specific filter against kinds that don't support
	...
	if _, supported := scopedWatchKinds[kind.Kind]; !supported && filter.GetScope() != "" {
	...
	case scopesv1.Mode_MODE_ANCESTORS, scopesv1.Mode_MODE_RELATIVES:
		...
		// NOTE: per the scoped security model, it would be permissible to allow unscoped callers to use thse
