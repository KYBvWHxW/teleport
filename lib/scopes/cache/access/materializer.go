...
type MaterializedAssignmentKey struct {
	...
	ListName string
	// ListScope is the scope of the access list the assignment is materialized for.
	ListScope string
}
...
func (m *Materializer) materializeAssignment(
	...
	m.logger.DebugContext(ctx, "Materializing scoped role assignment",
		"user", key.User,
		"list", accesslists.ScopeQualifiedName(list).String())
