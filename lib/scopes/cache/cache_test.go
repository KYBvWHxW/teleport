	...
	for _, tt := range tts {
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, item := range items {
				_, ok := cache.Get(item.Scope(), item.Key())
				require.False(t, ok, "item %+v should not be in cache before Put", item)

				cache.Put(item)

				got, ok := cache.Get(item.Scope(), item.Key())
...
func TestCacheOperations(t *testing.T) {
	...
	cache.Del("/child", "child-scoped-other")
	...
	cache.Del("/child/subchild", "child-sub-scoped")
